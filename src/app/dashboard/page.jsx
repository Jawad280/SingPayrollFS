...
const Dashboard = () => {
  ...
  const { data, error, isLoading } = useSWR(`http://localhost:3000/api/employees/companyName/${companyName}`, fetcher);
  ...
  const checker = async (employeeShare, employerShare, employee, contributionMonthYear) => {
    ...
    const res = await fetch(`http://localhost:3000/api/payslips/name/${name}/${NRIC}`);
    ...
    if (data) {
      ...
      const send = await fetch(`http://localhost:3000/api/payslips/${data.id}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
